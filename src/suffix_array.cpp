...
SuffixArray::SuffixArray(FILE* in, const char* text, size_t textLen) {
	...
	fread(&suffix_array_[0], sizeof(std::vector<saidx_t>::value_type), suffix_array_.size(), in);
}
...
void SuffixArray::saveSuffixArray(FILE* out) {
	...
	fwrite(&suffix_array_[0], sizeof(std::vector<saidx_t>::value_type), suffix_array_.size(), out);
}
