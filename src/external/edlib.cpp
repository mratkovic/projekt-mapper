...
int edlibCalcEditDistance(const unsigned char* query, int queryLength,
                          ...
                          int* bestScore, int** endLocations,
                          int** startLocations, int* numLocations,
  ...
  do {
    ...
    if (mode == EDLIB_MODE_HW || mode == EDLIB_MODE_SHW) {
      ...
      myersCalcEditDistanceNW(blocks, Peq, W, maxNumBlocks, query, queryLength,
                              target, targetLength, alphabetLength, k,
                              bestScore, &positionNW, findAlignment,
                              &alignData);
    }
    k *= 2;
  ...
  if (*bestScore >= 0) {  // If there is solution.
      ...
      if (mode == EDLIB_MODE_HW) {  // If HW, I need to calculate start locations.
        ...
        for (int i = 0; i < *numLocations; i++) {
          ...
          myersCalcEditDistanceSemiGlobal(
              ...
              alphabetLength, *bestScore, EDLIB_MODE_SHW, &bestScoreSHW,
    ...
    if (findAlignment) {
      ...
      obtainAlignment(maxNumBlocks, queryLength,
                      alnEndLocation - alnStartLocation + 1, W, *bestScore,
                      alnEndLocation - alnStartLocation, alignData, alignment,
                      alignmentLength);
    }
  }
  /*-------------------------------------------------------*/
...
static int myersCalcEditDistanceNW(Block* blocks, Word* Peq, int W,
  ...
  int lastBlock = min(
      maxNumBlocks,
      ceilDiv(min(k, (k + queryLength - targetLength) / 2) + 1, WORD_SIZE)) - 1;
  Block* bl;  // Current block
  ...
  for (int c = 0; c < targetLength; c++) {  // for each column
    ...
    if (lastBlock + 1 < maxNumBlocks && !(  //score[lastBlock] >= k + WORD_SIZE ||  // NOTICE: this condition could be satisfied if above block also!
      ...
      bl->P = (Word) -1;  // All 1s
      bl->M = (Word) 0;
      int newHout = calculateBlock(bl->P, bl->M, Peq_c[lastBlock], hout, bl->P,
                                   bl->M);
    ...
    if (c % STRONG_REDUCE_NUM == 0) {  // Every some columns do more expensive but more efficient reduction
      while (lastBlock >= firstBlock) {
        ...
        for (int i = 0; i < WORD_SIZE; i++) {
          ...
          if (scores[i] <= k
              && r <= k - scores[i] - targetLength + c + queryLength + W + 1) {
            reduce = false;
            break;
          }
          r--;
