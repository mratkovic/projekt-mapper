...
uint32_t LCSk::calcLCSk(uint32_t k,
...
uint32_t LCSk::calcLCSkpp(
  ...
  if (k > 1) {
    ...
    for (it = elements.begin(); it != elements.end(); ++it) {
      std::pair<uint32_t, uint32_t> prevElement = std::make_pair(
  ...
  for (std::vector<Event>::iterator event = events.begin();
